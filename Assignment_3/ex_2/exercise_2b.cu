...
void execute_on_GPU(time_t seed, size_t number_of_iterations, size_t number_of_particles, size_t block_size)
{
	printf("Computing particles on the GPU...\n");
	uint64_t start = time_ns();

	Particle* particles = 0;
	cudaMallocManaged(&particles, number_of_particles * sizeof(Particle));

	setup_data(particles, number_of_particles, seed);

	curandState* randoms = 0;
	cudaMallocManaged((void**)&randoms, number_of_particles * sizeof(curandState));

	for (size_t j = 0; j < number_of_iterations; j++)
	{
		simpleKernel << <(number_of_particles + block_size - 1) / block_size, block_size >> > (particles, number_of_particles, seed, randoms, j);
...
int main(int argc, char* argv[])
{
	...
	execute_on_GPU(seed, num_iterations, num_particles, block_size);
