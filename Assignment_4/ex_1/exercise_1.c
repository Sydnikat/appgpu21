...
__kernel \
void helloWorld() { \
	int global_index_x = get_global_id(0); \
	int global_index_y = get_global_id(1); \
	int global_index_z = get_global_id(2); \
										   \
	int group_index_x = get_group_id(0);   \
	int group_index_y = get_group_id(1);   \
	int group_index_z = get_group_id(2);   \
										   \
	int local_index_x = get_local_id(0);   \
	int local_index_y = get_local_id(1);   \
	int local_index_z = get_local_id(2);   \
										   \
	printf(\"Hello World! My GLOBAL coordinates are (%d,%d,%d). My GROUP coordinates are(%d,%d,%d). My LOCAL coordinates are (%d,%d,%d). \\n\", \
		global_index_x, global_index_y, global_index_z, group_index_x, group_index_y, group_index_z, local_index_x, local_index_y, local_index_z); \
} \
"; 
